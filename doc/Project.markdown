...
| 高级SQL（*）       | 2    | 应用trigger或function或view，需要自行学习SQL相关知识。       |
| 其他页面           | 1    | 等待页面/异常页面                                            |
| lighthouse（**）   | 4    | “无障碍”一项的分数÷25                                        |
| 页面美观程度       | 3    | 平均2分 按照正态                                             |
| ~~减少助教工作量~~ | 2    | 提前两周完成pj并参加面试                                     |

（*）的项需提交一篇小文档，说明其实现原理即可。**不要卷！！！**

（**）Lighthouse为Edge浏览器分析页面质量的工具。在Edge浏览器中，按下F12之后，在向右双箭头“更多选项卡”中可以找到。实际评分时，助教会选择如下选项进行评分：

![lighthouse](C:\Users\chen\Documents\Code\2023-OASD\doc\pic\lighthouse.png)



评分细则主要包含一些基本的标签有无**无障碍属性**（例如`img`的`alt`），还有**颜色搭配**的对比度等。**实际得分为`ceil(score / 25.0) `**。也就是说，**75分及以上的无障碍分数在本次项目中会视作满分**。然而，仍然鼓励大家拿高分，为人人皆能使用的，平等博爱内容丰富的互联网做出自己的第一份贡献。（~~注：B站的得分只有60分，非常差！它为了界面的炫酷，让盲人及视力障碍者赖以上网的屏幕阅读器感到十分的困惑！但按我们的算法也拿到了3分。~~）

## 文档

### DOs

文档的格式**必须为pdf**。使用类似于Typora一样的Markdown工具，能把你的富文本文档转成一个样子不差的，好看的pdf。

文档开头，必须**明确指出**后端端口及各接口的路径。

文档当中，如果实际项目中有实现，必须包含**上述评分细则中所要求阐述的项**。

如若没有实现对应功能但仍于文档当中阐述了加分的项，考虑到“心有余而力不足”的同学，将视情况酌情给分，然而不会超过该项分数的一半（不含小数部分）。大部分情况下，这个分数不会超过一分。**例如，一个3分的信息安全项，若能够说明攻击手段的大致原理以及防御措施，将给一分。**

文档如果有**清晰的思维脉络以及合理的结构**，那么附加分中的**页面美观程度**更有可能拿分，并且**期末面试分**就自然满分了。

### DON'Ts

**不能写得太长。**这不是论文！为了减轻大家的负担，请大家酌情阐述。不要有太多表达同一个意思的多句话！

**不能包含太多的截图。**网页是动态的、多样的，每个同学的操作逻辑、网页构建都不一样！所有的网页样式，以助教检查的时候为准。然而，对于一些必须的项目或者认为**无截图则难以阐明自己的工作量的项目**（例如：把自己的电脑变成服务器而不选择去租一台服务器，自己电脑中LAMP或LNMP各种配置的截图；图片局部放大时，阐述放大元素位置关系的截图等等），**鼓励放置截图**。

总之，如果两名助教一致认为“**这个人在卷文档而网页却没有什么实质性的内容**”，那么对于别人一般满分或几乎满分的项——他的**期末面试分**可能就危了。
